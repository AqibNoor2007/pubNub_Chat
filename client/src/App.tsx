...
// let "john" = generateName();
 
const pubnub = new PubNub({
  publishKey: 'pub-c-375dc634-0fb3-481c-9a6a-1de0947f7558',
  subscribeKey: 'sub-c-5ca2ed0b-1ac6-45cc-87e2-7bf6185f2656',
  uuid: "john",
  presenceTimeout: 20
 
});
const channelPrefix = 'supportChannel.';
const channel = channelPrefix+"john" ;
...
const SupportClient = () => {
  ...
  const [activeChatChannel, setActiveChatChannel] = useState('supportChannel.agent');
    ...
    async message => {
      if (input) {
        await pubnub.publish({
          message: {sender:"client",name:"john",message:input.replace(/<[^>]*>?/gm, '')},
          channel: channel
        });
        console.log(message)
            ...
            <MessageList   style={{ overflow: "auto" }}>
              {messages.map((message, messageIndex) => {
                if (message.sender === "agent") {
                  ...
                  return (
                    <NewMessage key={`message-${messageIndex}`}>
                      <ClientName>{"john"} (You)</ClientName>
